...
/* 	
	Vars Globais
    	...
    	As vars globais são as vars declaradas fora de qualquer
	função (antes da função main).

	Isto significa que o seu âmbito (scope) é visível em 
	todo o programa, podendo assim ser acedidas, alteradas
	e partilhadas por qualquer parte do programa, incluindo,
 	naturalmente funções.

	Desta forma, uma var global sendo alterada é efetivamente
	alterada em todo o programa, dado que o conhecimento do 
	seu valor é global.

	Não devem ser redeclaradas novamente dentro de funções,
	já que causam conflito de declarações (e de âmbito).

	Vejamos os exemplos constantes neste programa.
*/

// declaração das vars globais
int x = 10;
int y = 20;
...
// Como o seu acesso é universal, esta acção vai ser 
// efetivamente partilhável por todo o programa.
