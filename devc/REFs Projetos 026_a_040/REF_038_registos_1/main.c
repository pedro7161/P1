...
// Estruturas são técnicas de programação que permitem 
// o armazenamento agrupado de vários tipos de dados.

// O seguinte exemplo mostra como utilizar structs
// (registos) 
//
// calcula a média de dois salarios
//
...
int main()
{
	...
	struct empregado // declara struct do tipo empregado
	{
		...
		char nome[30];
		float salario;
	}emp1, emp2; // as vars da struct para podermos acedê-la
 
/*
	As linhas de código acima podem ser redeclaradas 
	da seguinte maneira:
...
//	fpurge(stdin); para Macs
	...
	printf("\nInsira salário de emp1: ");
	fflush(stdin);
	scanf("%f", &emp1.salario);

	printf("\n\nInsira número de emp2: ");
	fflush(stdin);
	scanf("%ld", &emp2.num);
	...
	printf("\nSalário de emp1: %4.2f", emp1.salario);

	printf("\n\n\n");
	printf("Num de emp2: %ld", emp2.num);
	printf("\nNome de emp2: %s", emp2.nome);
	printf("\nSalário de emp2: %4.2f", emp2.salario);
