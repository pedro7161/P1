...
// Este programa aplica o método de pesquisa binária
...
// chamado de base, que serve assim de referência.

// A pesquisa binária requer o array-base ordenado por
// ordem crescente, caso contrário este método simplesmente
// não funciona.

// macro que calcula o número de elementos num array.
// Determina dividindo o num de bytes no array pelo num
// de bytes ocupado por cada elmento.

#define n_els(x) (sizeof(x) / sizeof(x[0]))

// a função recebe args do tipo ptr e devolve 
...
// recebe o endereço do elm do array chaves
// a pesquisar no array base

	int *target;

// calcula aqui os tamnhos atrvés das macros
...
// endereço da primeira chave, onde está o valor 2
		target = chaves + i;

		printf("\n\n\n\nEndereco do Valor %d a ser testado = %ld\n", *target, target);
			 
// chamada da função
// lembrar que este método devolve um ptr para
// um intero.
...
// se o valor for encontrado, o seu endereco é 
// devolvido pela func e armazenado em res
// caso contrário res recebe um valor false (0),
// mas tem que ser do tipo ptr

		res = pesquisa_binaria(target, base, tam_base);
		
		if (res) // se ptr não for null
		{			
